                ...
                <div v-for="item in items" :key="item.id" class="col-sm-3" v-bind:href="item.id">
                    <div class="card">
                        ...
                        <div class="card-block">
                            ...
                            <p class="card-value">{{item.discount_amount}}  <span class="italic">₺</span></p></p>
...
<script>
    export default {
        ...
        methods: {
            fetchItems(page_url) {
                let vm = this;
                page_url = (page_url)  ? page_url : 'https://gorest.co.in/public-api/products?page=1';
                fetch(page_url)
                    .then(res => res.json())
                    .then(res => {
                        this.items = res.data;
            ...
            makePagination(meta) {
                ...
                let prev =  ((metaPage==1) ? null :  "https://gorest.co.in/public-api/products?page="+(metaPage-1)) ;
                let next =  ((metaPage+1<=meta.pages) ? "https://gorest.co.in/public-api/products?page="+ (metaPage+1) : null);
